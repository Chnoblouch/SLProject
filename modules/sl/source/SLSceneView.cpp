...
void SLSceneView::onInitialize()
{
    ...
    if (gl3wIsSupported(4, 4))
    {
        ...
        if (_s && _s->root3D() && conetracer())
            conetracer()->init(_scrW, _scrH, _s->root3D()->aabb()->minWS(), _s->root3D()->aabb()->maxWS());
    }
#endif
...
void SLSceneView::onResize(SLint width, SLint height)
{
    ...
    if (_scrW != width || _scrH != height)
    {
        ...
        if (_renderType != RT_gl)
        {
            renderType(RT_gl);
            // TODO: MOVE INTO RENDERER
            _rendererRT->raytracer()->doContinuous(false);
        }
    }
}
//-----------------------------------------------------------------------------
                              ...
                              SLKey         mod)
{
    ...
    // TODO: MOVE INTO RENDERER
    if (_rendererRT->raytracer()->state() == rtMoveGL)
    {
        renderType(RT_rt);
        _rendererRT->raytracer()->state(rtReady);
    }

    // Continue with path tracing
    if (_rendererPT->pathtracer()->state() == rtMoveGL)
    {
        renderType(RT_pt);
        _rendererPT->pathtracer()->state(rtReady);
    }
...
SLbool SLSceneView::onMouseMove(SLint scrX, SLint scrY)
{
    ...
    if (_mouseDownL || _mouseDownR || _mouseDownM)
    {
        ...
        // TODO: MOVE INTO RENDERER
        if (_renderType == RT_rt && !_rendererRT->raytracer()->doContinuous())
        {
            if (_rendererRT->raytracer()->state() == rtFinished)
                _rendererRT->raytracer()->state(rtMoveGL);
            else
                _rendererRT->raytracer()->doContinuous(false);
        ...
        {
            if (_rendererPT->pathtracer()->state() == rtFinished)
                _rendererPT->pathtracer()->state(rtMoveGL);
...
SLbool SLSceneView::onMouseWheel(SLint delta, SLKey mod)
{
    ...
    // TODO: MOVE INTO RENDERER
    if (_renderType == RT_rt && !_rendererRT->raytracer()->doContinuous() &&
        _rendererRT->raytracer()->state() == rtFinished)
        _rendererRT->raytracer()->state(rtReady);

    // Handle mouse wheel in PT mode
    if (_renderType == RT_pt && _rendererPT->pathtracer()->state() == rtFinished)
        _rendererPT->pathtracer()->state(rtReady);
...
SLbool SLSceneView::onKeyPress(SLKey key, SLKey mod)
{
    ...
    if (key=='R') {renderType(RT_rt);}
    if (key=='P') {renderType(RT_pt);}
#ifdef SL_HAS_OPTIX
...
SLstring SLSceneView::windowTitle()
{
    ...
    return profiling + _renderer->windowTitle() + profiling;
}
//-----------------------------------------------------------------------------
#ifdef SL_HAS_OPTIX
...
{
    _rendererCT = new SLRendererConetracing(this, shaderDir);
}
//-----------------------------------------------------------------------------
void SLSceneView::updateRenderer()
{
    SLRenderer* lastRenderer = _renderer;

    switch (_renderType)
    {
        case RT_gl: _renderer = _rendererGL; break;
        case RT_rt: _renderer = _rendererRT; break;
        case RT_pt: _renderer = _rendererPT; break;
        case RT_ct: _renderer = _rendererCT; break;
        default: _renderer = nullptr;
    }

    if (_renderer && lastRenderer != _renderer)
    {
